...
const whyBuyItems = siteConfig.benefits.map((benefit) => ({
  ...
  icon: iconMap[benefit.title as keyof typeof iconMap] || (
    <ShieldCheck className="w-8 h-8 text-soft-black" />
  ),
}));
    ...
    <section className="bg-white py-24 px-6 border-t border-black/5">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="text-center mb-20">
          <p className="font-sans text-[11px] md:text-xs tracking-[0.3em] text-gray-800 uppercase mb-6">
            Why Collect With Us
          </p>
          <h2 className="font-serif text-4xl md:text-5xl text-soft-black leading-none">
            THE SHAKYA <span className="italic">PROMISE</span>
          </h2>
        </div>
        ...
        >
          {whyBuyItems.map((item, index) => (
            ...
            >
              <div className="p-4 bg-bone rounded-full mb-2">{item.icon}</div>
              <h3 className="font-serif text-xl text-soft-black">
                {item.title}
              </h3>
              <p className="font-sans text-sm text-soft-black leading-relaxed max-w-xs">
                {item.description}
              </p>
            </motion.div>
          ))}
