...
export function useArtFilter(artworks: Artwork[], initialCategory: string | null = null) {
  ...
  const statusFilter = (searchParams.get("status") || "all") as FilterOptions["statusFilter"];
  const sortOption = (searchParams.get("sort") || "newest") as FilterOptions["sortOption"];
